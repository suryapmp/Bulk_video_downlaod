...
# ---------- Globals ----------
TASKS: List[Dict[str, Any]] = []   # each: {exam_cycle,batch,sem,program,course,roll,url,exam_date}
clients = set()                    # ws clients
total_downloaded_bytes = 0         # total bytes downloaded (session)
...
def sanitize_filename(name: str) -> str:
    if name is None:
        return ""
    name = str(name)
    # remove unsafe chars, replace slashes/newlines with underscore
    return re.sub(r'[<>:"/\\|?*\n\r]+', "_", name).strip()
...
async def upload(file: UploadFile = File(...)):
    ...
    Required columns (case-insensitive): exam_cycle, batch, sem, program_code, course_name, link, roll_no, exam_date
    ...
    # required columns (user asked to include exam_cycle, batch, sem)
    required = {"exam_cycle", "batch", "sem", "program_code", "course_name", "link", "roll_no", "exam_date"}
    if not required.issubset(set(df.columns)):
        return JSONResponse({"error": f"Excel must contain columns: {required}"}, status_code=400)

    rows_for_ui = []
    for _, row in df.iterrows():
        exam_cycle = str(row.get("exam_cycle", "")).strip()
        ...
        # parse/clean exam_date: remove time portion if present and produce YYYY-MM-DD or clean string
        ...
        if pd.isna(exam_raw):
            ...
            try:
                if isinstance(exam_raw, (pd.Timestamp, datetime.datetime, datetime.date)):
                    ...
                    # try parsing common datetime-like strings and keep only date
                    parsed = pd.to_datetime(str(exam_raw), errors="coerce")
                    if not pd.isna(parsed):
                        exam_date = parsed.strftime("%Y-%m-%d")
                    else:
                        exam_date = str(exam_raw).strip()
                        # remove time-like portions (e.g., "2024-07-01 unknown_date")
                        exam_date = re.split(r"\s+", exam_date)[0]
                        exam_date = raw_date.split()[0].replace("_", "-")
            except Exception:
                exam_date = str(exam_raw).strip().split()[0].replace("/", "-")
...
def preview(fp: str):
    ...
    # Return the file directly (streaming), browser will play it
    return FileResponse(fp_abs, media_type="video/mp4", filename=os.path.basename(fp_abs))

@app.get("/download")
def download(fp: str):
    """
    Download a file as attachment. Same allowed roots check as /preview.
    Returns Content-Disposition: attachment so the browser saves the file.
    """
    try:
        fp_un = unquote(fp)
    except Exception:
    ...
    return FileResponse(fp_abs, media_type="application/octet-stream", filename=os.path.basename(fp_abs))
...
async def download_one(session: aiohttp.ClientSession, task: Dict[str, Any], base_path: str,
    ...
    Folder structure:
        <base_path>/<exam_cycle>/<batch>/<sem>/<program_code>/<course_name>/<exam_date>/<roll_no>.mp4
    """
    global total_downloaded_bytes

    exam_cycle = sanitize_filename(task.get("exam_cycle", ""))
    batch = sanitize_filename(task.get("batch", ""))
    sem = sanitize_filename(task.get("sem", ""))
    program = sanitize_filename(task.get("program", ""))
    course = sanitize_filename(task.get("course", ""))
    exam_date = sanitize_filename(task.get("exam_date", "unknown_date"))
    roll = sanitize_filename(task.get("roll", "unknown_roll"))
    url = task.get("url")

    # build folder structure
    dest_dir = os.path.join(base_path, exam_cycle, batch, sem, program, course, exam_date)
    ensure_dir(dest_dir)
    ...
    download_url = "/download?fp=" + quote(os.path.abspath(filepath))
    ...
    await broadcast({
        ...
        "status": "Starting", "progress": 0 if not downloaded else 100, "speed": "-",
        "size": f"{downloaded / (1024 ** 2):.2f} MB" if downloaded else "-",
        "preview": None if downloaded == 0 else preview_url,
        "download": None if downloaded == 0 else download_url,
        "total": total_videos,
    ...
    async with semaphore:
        try:
            ...
            await broadcast({
                ...
                "preview": preview_url, "download": download_url,
...
async def download_all(tasks: List[Dict[str, Any]], base_path: str, concurrency_value: int):
    ...
    async with aiohttp.ClientSession() as session:
        # prepare tasks; each download_one will skip/resume if appropriate
